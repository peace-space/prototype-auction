...
import 'package:prototype_your_auction_services/channel/ProductDetailChannel.dart';
...
class UserAuctionDetailAdminWidgetState
  ...
  ProductDetailModel auctionDetailAdminModel = ProductDetailModel();
  late Map auction_data = auctionDetailAdminModel.getAuctionDetailAdminData();

  @override
  void dispose() {
    ProductDetailChannel.closeAdmin(id_auctions: auction_data['id_users']);
    super.dispose();
  }
        ...
        builder: (context, snapshot) {
          ...
          if (snapshot.hasData) {
            try {
              ...
              dynamic data = ProductDetailModel.getConvertToMap();
              if (data == null) {
                return Center(
                  child: CircularProgressIndicator(),
                );
